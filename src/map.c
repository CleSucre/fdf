...
/*   Updated: 2024/05/01 06:53:29 by julthoma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_pregen_horizontal_line(t_map *map, t_win *win, int i, int j)
{
	...
	{
		transform_point(
			map->transformed_point1, &map->map_vector3[i][j], map->camera);
		transform_point(
			map->transformed_point2, &map->map_vector3[i][j + 1], map->camera);
		ft_projet_vector3(
			map->line->point1, map->transformed_point1, map->camera);
		ft_projet_vector3(
			map->line->point2, map->transformed_point2, map->camera);
		if (ft_check_frustum(map->transformed_point1)
			&& ft_check_frustum(map->transformed_point2))
...
static void	ft_pregen_vertical_line(t_map *map, t_win *win, int i, int j)
{
	if (map->map_vector3[i][j].z >= 0 && map->map_vector3[i + 1][j].z >= 0)
	{
		transform_point(
			map->transformed_point1, &map->map_vector3[i][j], map->camera);
		transform_point(
			map->transformed_point2, &map->map_vector3[i + 1][j], map->camera);
		ft_projet_vector3(
			map->line->point1, map->transformed_point1, map->camera);
		ft_projet_vector3(
			map->line->point2, map->transformed_point2, map->camera);
